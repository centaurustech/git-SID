<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('companies', "create table if not exists `companies` (   `company_id` TINYINT not null auto_increment , primary key (`company_id`), `name` VARCHAR(60) , `client` TINYINT , `industry` VARCHAR(40) , `company_number` INT , `country_hq` VARCHAR(40) , `country_operations` VARCHAR(40) , `num_employees` INT , `company_type` VARCHAR(40) , `sic_code` INT , `created` DATE , `created_by` VARCHAR(40) ) CHARSET utf8", $silent);
		...
		setupTable('reports', "create table if not exists `reports` (   `report_id` TINYINT not null auto_increment , primary key (`report_id`), `date` DATE , `company` TINYINT , `created` DATE , `created_by` VARCHAR(40) ) CHARSET utf8", $silent);
		setupIndexes('reports', array('company'));
		setupTable('entries', "create table if not exists `entries` (   `entry_id` INT not null auto_increment , primary key (`entry_id`), `report` TINYINT , `outcome_area` INT , `outcome` INT , `beneficiary_group` TINYINT , `beneficiary_group_relevance` VARCHAR(40) , `created` DATE , `created_by` VARCHAR(40) , `comment` MEDIUMTEXT , `reference` VARCHAR(255) , `reliability` VARCHAR(40) , `intentionality` VARCHAR(40) , `equivalence` VARCHAR(40) , `score` TINYINT ) CHARSET utf8", $silent);
		setupIndexes('entries', array('report','outcome','beneficiary_group'));
